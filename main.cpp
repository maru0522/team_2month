...
#pragma region InitDirectX初期化

    mInitDirectX* mIDX = mInitDirectX::GetInstance();
    mIDX->Initialize();
...
#pragma region srv
    ...
    result = mIDX->GetDevice()->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&srvHeap));
...
#pragma region 描画初期化処理
    ...
    result = mIDX->GetDevice()->CreateCommittedResource(
        &heapProp, // ヒープ設定
...
#pragma region グラフィックスパイプライン設定
    ...
    assert(SUCCEEDED(result));
    result = mIDX->GetDevice()->CreateRootSignature(0, rootSigBlob->GetBufferPointer(), rootSigBlob->GetBufferSize(),
...
#pragma region パイプラインステートの生成
    ...
    result = mIDX->GetDevice()->CreateGraphicsPipelineState(&pipelineDesc, IID_PPV_ARGS(&pipelineState));
    assert(SUCCEEDED(result));
#pragma endregion
...
#pragma endregion 
    ...
    for (int i = 0; i < _countof(object3ds); i++) {
        // 初期化
        InitializeObject3d(&object3ds[i], mIDX->GetDevice());
...
#pragma region テクスチャマッピング
    ...
    mIDX->GetDevice()->CreateShaderResourceView(texBuff.Get(), &srvDesc, srvHandle);


    // 1つハンドルを進める
    UINT incrementSize = mIDX->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    mIDX->GetDevice()->CreateShaderResourceView(texBuff2.Get(), &srvDesc2, srvHandle);
#pragma endregion
...
#pragma region 描画コマンド
        ...
        mIDX->GetCommandList()->SetPipelineState(pipelineState.Get());
        mIDX->GetCommandList()->SetGraphicsRootSignature(rootSignature.Get());

        // プリミティブ形状の設定コマンド
        mIDX->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); // 三角形リスト

        // 頂点バッファビューの設定コマンド
        mIDX->GetCommandList()->IASetVertexBuffers(0, 1, &vbView);

        // インデックスバッファビューの設定コマンド
        mIDX->GetCommandList()->IASetIndexBuffer(&ibView);


        mIDX->GetCommandList()->SetGraphicsRootConstantBufferView(0, constBuffMaterial->GetGPUVirtualAddress());
        // SRVヒープの設定コマンド
        // // デスクリプタヒープの配列
        ID3D12DescriptorHeap* ppHeaps[] = { srvHeap.Get() };
        mIDX->GetCommandList()->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);
        ...
        mIDX->GetCommandList()->SetGraphicsRootDescriptorTable(1, srvGpuHandle);

        // 全オブジェクトについて処理
        for (int i = 0; i < _countof(object3ds); i++) {
            DrawObject3d(&object3ds[i], mIDX->GetCommandList(), vbView, ibView, _countof(indices));
        }
